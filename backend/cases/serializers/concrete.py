...
class DeathsRealDailyNormalisedSerializer(serializers.ModelSerializer):
    """Serializer for real daily normalised deaths"""

    iso_code = serializers.PrimaryKeyRelatedField(source="country", read_only=True)
    weekly_avg_deaths_per_million = serializers.SerializerMethodField()
    summed_avg_deaths_per_million = serializers.SerializerMethodField()

    def get_weekly_avg_deaths_per_million(self, record):  # pylint: disable=no-self-use
        """Getter for weekly_avg_deaths_per_million field"""
        if record.country.population and record.country.population != 0:
            return 1e6 * record.weekly_avg_deaths / record.country.population
        return 0

    def get_summed_avg_deaths_per_million(self, record):  # pylint: disable=no-self-use
        """Getter for summed_avg_deaths_per_million field"""
        if record.country.population and record.country.population != 0:
            return 1e6 * record.summed_avg_deaths / record.country.population
        return 0
    ...
    class Meta:
        ...
        fields = (
            "iso_code",
            "date",
            "weekly_avg_deaths_per_million",
            "summed_avg_deaths_per_million",
        )


class DeathsRealIntegratedSerializer(serializers.ModelSerializer):
    """Serializer for real daily integrated deaths"""

    iso_code = serializers.SerializerMethodField()
    date = serializers.SerializerMethodField()
    summed_avg_deaths = serializers.SerializerMethodField()
    summed_avg_deaths_per_million = serializers.SerializerMethodField()
    ...
    def get_summed_avg_deaths(self, datadict):  # pylint: disable=no-self-use
        """Getter for summed_avg_cases field"""
        return datadict["summed_avg_deaths"]

    def get_summed_avg_deaths_per_million(
        self, datadict
    ):  # pylint: disable=no-self-use
        """Getter for summed_avg_cases_per_million field"""
        return datadict["summed_avg_deaths_per_million"]
