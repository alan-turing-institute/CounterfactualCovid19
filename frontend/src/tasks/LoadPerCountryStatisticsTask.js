...
class LoadPerCountryStatisticsTask {
  loadIntegratedCasesAllCountries = async (end_date) => {
    try {
      const target = `/api/cases/real/integrated/?end_date=${end_date}`;
  ...
  loadIntegratedCases = async (iso_code, start_date, end_date) => {
    try {
      const target = `/api/cases/real/integrated/?iso_code=${iso_code}&start_date=${start_date}&end_date=${end_date}`;
      console.debug(`Backend ${target}`);
      const response = await axios.get(target, {});
  ...
  loadIntegratedCounterfactualCases = async (
    ...
    try {
      var target = `/api/cases/counterfactual/integrated/?iso_code=${iso_code}&start_date=${start_date}&end_date=${end_date}`;
  ...
  loadIntegratedDeaths = async (iso_code, start_date, end_date) => {
    try {
      const target = `/api/deaths/real/integrated/?iso_code=${iso_code}&start_date=${start_date}&end_date=${end_date}`;
      console.debug(`Backend ${target}`);
      const response = await axios.get(target, {});
      return response.data[0];
    } catch (error) {
